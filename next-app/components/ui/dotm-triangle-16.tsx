...
import { cx, remapOpacityToTriplet, styleOpacity, stylePx, type DotMatrixCommonProps } from "@/lib/dotmatrix-core";
...
export function DotmTriangle16({
  ...
  const trackSize =
    cellPadding == null
      ? (size - gap * (MATRIX_SIZE - 1)) / MATRIX_SIZE
      : dotSize;
  const visibleSpan =
    cellPadding == null ? size + Math.max(0, dotSize - trackSize) : matrixSize;
  const rootStyle = {
    width: stylePx(visibleSpan),
    height: stylePx(visibleSpan),
    color
  } as CSSProperties;
