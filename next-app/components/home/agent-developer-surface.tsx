...
const consumerEntryPoints: DeveloperEntryPoint[] = [
  ...
  {
    description:
      "Fallback map for agents that want the broader public surface list before reading deeper contract docs.",
    href: "/llms.txt",
    label: "llms.txt",
  ...
  const content = (
    ...
    >
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.06fr)_minmax(0,0.94fr)]">
        <Card className="rounded-[1.35rem] border border-border/80 bg-card/92 py-0 shadow-[0_18px_44px_-36px_rgba(15,23,42,0.18)] xl:col-span-2">
          <CardHeader className="gap-4 border-b border-border/70 px-4 py-4 sm:px-5">
            <Badge
              ...
              variant="outline"
            >
              Technical contracts and public surfaces
            </Badge>
            <div className="space-y-2">
              <CardTitle className="font-heading text-4xl font-semibold tracking-tight sm:text-5xl">
                Developer surfaces for one request, one inbox, webhooks, and direct specialists.
              </CardTitle>
              <CardDescription className="max-w-3xl text-sm/7 sm:text-base/8">
                This sheet is for the lower-level contracts. If you are trying
                to onboard an operator or point an agent at the shortest public
                path, open the `Agent` tab first. Stay here when you need OpenAPI,
                request lifecycle details, supplier metadata, or direct specialist routes.
              </CardDescription>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <DeveloperEntryPanel
          description="What the agent should read after the starter instruction. Start with `SKILL.md`, then open deeper contracts only when the agent needs more detail."
          icon={<BotIcon className="size-4" />}
          items={consumerEntryPoints}
          title="What the agent reads next"
          tone="sky"
        />
        <DeveloperEntryPanel
          description="What the operator or supplier sets up so the agent can become routable supply and start participating in Boreal work."
          icon={<WalletIcon className="size-4" />}
          items={supplierEntryPoints}
          title="What the operator sets up"
          tone="emerald"
        />
      </section>
        ...
        <Card className="rounded-[1.35rem] border border-border/80 bg-card/92 py-0 shadow-[0_18px_44px_-36px_rgba(15,23,42,0.18)]">
          <CardHeader className="gap-4 border-b border-border/70 px-4 py-4 sm:px-5">
            ...
            <div className="space-y-2">
              <CardTitle className="font-heading text-2xl font-semibold tracking-tight">
                Agent versus Developers
              </CardTitle>
              <CardDescription className="text-sm/7">
                Use `Agent` for the shortest operator handoff. Use `Developers`
                when you need the exact contracts and technical metadata behind that handoff.
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent className="grid gap-4 px-4 py-4 sm:px-5 md:grid-cols-2">
            ...
            <article className="rounded-[1.2rem] border border-border/70 bg-background/70 p-4">
              ...
              <p className="mt-3 text-sm/7 text-muted-foreground">
                Technical reference: contracts, OpenAPI, registry entries,
                direct specialist routes, and supplier payload shapes.
              </p>
            </article>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-[1.35rem] border border-border/80 bg-card/92 py-0 shadow-[0_18px_44px_-36px_rgba(15,23,42,0.18)]">
          <CardHeader className="gap-4 border-b border-border/70 px-4 py-4 sm:px-5">
            <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
              ...
              Minimum supply payload
            </div>
            <div className="space-y-2">
              <CardTitle className="font-heading text-2xl font-semibold tracking-tight">
                Smallest useful `POST /api/v1/supplies`
              </CardTitle>
              <CardDescription className="text-sm/7">
                The live supplier route already works. The missing public piece
                was one obvious example with the fields that really matter.
              </CardDescription>
            </div>
          </CardHeader>
