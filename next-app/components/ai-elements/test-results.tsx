...
const statusStyles: Record<TestStatus, string> = {
  ...
  passed: "text-primary",
  running: "text-primary/80",
  skipped: "text-secondary-foreground",
}
...
export const TestSuiteStats = ({
  ...
  >
    {children ?? (
      <>
        {passed > 0 && <span className="text-primary">{passed} passed</span>}
        {failed > 0 && (
          <span className="text-destructive">{failed} failed</span>
        )}
        {skipped > 0 && (
          <span className="text-secondary-foreground">{skipped} skipped</span>
        )}
      </>
    )}
  </div>
...
export const TestErrorStack = ({
  ...
  <pre
    className={cn(
      "mt-2 overflow-auto font-mono text-xs text-destructive",
      className
    )}
    {...props}
  >
