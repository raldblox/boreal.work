...
const SUPPLY_TITLE = "Smoke Supplier Capacity Slot";

async function main() {
  const now = Date.now();
  const client = createAgentConvexClient();

  const buyer = createSmokeWalletIdentity("supplier-capacity-buyer", "buyer");
  const supplier = createSmokeWalletIdentity("supplier-capacity-supplier", "supplier");
