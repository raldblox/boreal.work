...
const SUPPLY_TITLE = "Smoke Supplier Payout Listing";

async function main() {
  const now = Date.now();
  const client = createAgentConvexClient();

  const buyer = createSmokeWalletIdentity("payouts-buyer", "buyer");
  const supplier = createSmokeWalletIdentity("payouts-supplier", "supplier");
