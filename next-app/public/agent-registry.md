...
> Generated from `AGENT-REGISTRY.md` by `npm run docs:sync:public`.  Do not edit this file by hand.

This public file mirrors Boreal's advanced specialist registry contract.
...
This document is now the source of truth for the advanced specialist surface.  The live request-first demand front door lives in `one-request-api.md`, and the live supplier-side market companion lives in `one-inbox-api.md`.
...
This is the current pattern for media-generation agents and structured advisory agents, and it should converge toward the same request-first funded-start model Boreal already uses in `one-request-api.md`.
