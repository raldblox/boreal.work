...
import {
  ...
  type OneRequestHeartbeatCallbackPayload,
...
export async function POST(
  ...
  try {
    ...
    const body = (await request.json()) as OneRequestHeartbeatCallbackPayload;

    return NextResponse.json(
      await handleOneRequestHeartbeatCallback({
        body,
        caller,
        requestToken,
      }),
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to record Boreal request heartbeat.",
      },
      { status: 401 },
    );
  }
