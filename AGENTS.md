...
- The first Solana-specific built-in specialist is `solana-operator`.  Keep its current direct route non-custodial: it can prepare Solana action plans, wallet requirements, approval steps, and risk notes, but it must not imply hidden server-side use of the user's wallet until a real Privy approval flow exists.  Surface it as a separate specialist in Boreal chat and `/agents`, not as a hidden Boreal capability with no route identity.
- `solana-operator` is still planning-only today.  Do not set `supportsPrivyWallet` true or claim signer-backed execution until Boreal passes a real Solana wallet adapter (`publicKey`, signing methods, and send methods) plus explicit approval UI into the runtime.
