...
- For the app in `next-app/`, use `npm run convex:reset:dev` when you want a clean dev deployment plus the built-in seeded agents restored in one step.  It should resolve the current selected Convex dev deployment once, wipe it, and seed that exact same target instead of trusting a drift-prone env default.
...
- When `solana-operator` is mounted from `Offers`, the chat intro should surface click-to-fill starter prompts for the shipped safe paths first: memo recording, message signing, simple SOL transfer, swap or stake planning, and wallet-safety review.
